...
'''
Create an audio file as it is generated by RP2A03 DPCM function
'''
...
def readWav(inFileName):
    '''
    Read audio file.
    It must be uncompressed, 16-bit sample wav file.
    Number of channels is 1 or 2.
    If number of channels is 2, it reads only left channel.
    ...
    if wr.getcomptype() != "NONE":
        ...
        usage()
        quit()
    # Number of channel is either 1 or 2
    ...
    while i<len(numData):
        ...
        # If the number of channel is 2, get only Left channel
        i += channels
...
def convDpcm(inData, inSampFreq, inRegId):
    ...
    '''
    Create a sound generated from DPCM channel.
    ...
    Procedure
     1. Create a sample value after compression
      ...
      2nd sample or later:
       - Compress the sample value (16 bit) to 7 bits.
       - Compare it with the previous compressed value
         - If new compressed value is larger than previous one, add one to the previous value.
         - If new compressed value is the same as or smaller than previous one, subtract one from the previous value.
      2. Convert to audio
       ...
       Them, shift left by 9 bits (=16-9) to restore the number of quantization bits to 16.
    ...
    # Determine sampling frequency of DPCM output
    ...
    while t < totalTime:
        ...
        if t <= 0.0:
            # When it's first sampe,
            # Compress sample tnto 7 bits
            # Python's shift operation is "mathmatical shift", so moving 9 bits (=16-7) to right works
            val = sample >> 9
        else:
            # When it's second sample or later
            # Compress sample
            tmpVal = sample >> 9
            # If compressed sample is larger than previous sample, add one to the previous sample.
    ...
    # Compressed sample is in 7 bits, and the last 1 bit will be ignored in the volume control, so after deleting the last 1 bit, convert to 16-bit signal.
